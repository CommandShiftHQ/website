...
```
Rules switched off `[0]` <br/> 
Besides allowing us freedom to use `.js` or `.jsx` extensions, we are also allowing ourselves to use prop spreading like `{...props}` and `underscore dangles` as we will use a variable named `_id` further on the track. 

Rules set to Warning `[1]` <br/>
